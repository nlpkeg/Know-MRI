...
default_loc = default_data_raw_dir/"Zhujiu-MCScript.json"

dataset_info = {
        "name": "Zhujiu-MCScript", 
        "des": """Zhujiu MCScript dataset contains script knowledge questions from Zhujiu Knowledge.""",
        "dataset_type": ""
    }

class ZhujiuMCScript(Dataset):
...
def get_processed_kvs(sample, keys=support_template_keys):
    ...
    kvs["prompt"] = sample["specific_goal"]
    kvs["ground_truth"] = " ".join(sample["script"])
    return kvs


def get_default_dataset():
    return ZhujiuMCScript(loc=default_loc)
