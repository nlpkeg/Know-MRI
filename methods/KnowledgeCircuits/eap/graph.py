...
class Graph:
    ...
    def to_json(self, filename: str, full_json: bool = False):
        """Save the graph to a json file
        Args:
            filename (str): the filename to save the graph to
            full_json (bool): if True, save all nodes and edges; if False, only include those with in_graph=True
        """
        # Prepare nodes and edges based on the full_json flag
        if full_json:
            nodes = {str(name): bool(node.in_graph) for name, node in self.nodes.items()}
            edges = {
                str(name): {
            ...
            # Only include nodes where in_graph is True
            nodes = {
                str(name): bool(node.in_graph)
                for name, node in self.nodes.items()
                if node.in_graph
            }
            # Only include edges where in_graph is True
            edges = {
                str(name): {
                    'score': None if edge.score is None else float(edge.score),
                    'in_graph': bool(edge.in_graph)
